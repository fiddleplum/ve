...
namespace ve
{
	...
	unsigned int getChar(std::string const & subject, size_t & i)
	{
		...
		if (i < subject.length())
		{
			if ((unsigned char)subject[i] <= 0x7f) // 0 to 7f
			{
				codePoint = subject[i] & 0x7f;
				i += 1;
			}
			else if (((unsigned char)subject[i] & 0xe0) == 0xc0 && i + 1 < subject.length()) // 80 to 7ff
			{
				codePoint = ((unsigned int)(subject[i] & 0x1f) << 6) + (unsigned int)(subject[i + 1] & 0x3f);
				i += 2;
			}
			else if (((unsigned char)subject[i] & 0xf0) == 0xe0 && i + 2 < subject.length()) // 800 to ffff
			{
				codePoint = ((unsigned int)(subject[i] & 0x0f) << 12) + ((unsigned int)(subject[i + 1] & 0x3f) << 6) + (unsigned int)(subject[i + 2] & 0x3f);
				i += 3;
			}
			else if (((unsigned char)subject[i] & 0xf8) == 0xf0 && i + 3 < subject.length()) // 10000 to 1fffff
			{
			...
			else if (((unsigned char)subject[i] & 0xfc) == 0xf8 && i + 4 < subject.length()) // 200000 to 3ffffff
			{
